package com.junhyeoklee.som.services;
...
public class SleepReminderService extends Service {
            ...
            if (nextAlarm != null) {
                ...
                startForeground(540, builder.setContentTitle(getString(R.string.title_sleep_reminder))
                        .setContentText(String.format(getString(R.string.msg_sleep_reminder),
                                AlarmFormatUtils.formatUnit(this, (int) TimeUnit.MILLISECONDS.toMinutes(nextAlarm.getNext().getTimeInMillis() - System.currentTimeMillis()))))
    ...
    public static AlarmData getSleepyAlarm(Alarmio alarmio) {
        if (AlarmPreferenceData.SLEEP_REMINDER.getValue(alarmio)) {
            AlarmData nextAlarm = getNextWakeAlarm(alarmio);
            if (nextAlarm != null) {
                Calendar nextTrigger = nextAlarm.getNext();
                nextTrigger.set(Calendar.MINUTE, nextTrigger.get(Calendar.MINUTE) - (int) TimeUnit.MILLISECONDS.toMinutes((long) AlarmPreferenceData.SLEEP_REMINDER_TIME.getValue(alarmio)));
